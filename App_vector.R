...
server <- function(input, output, session) {
  ...
  observeEvent(input$calculate, {
    ...
    # Downsample the raster to a lower resolution when rendering to leaflet map
    ##This is done to avoid getting the out of memory error when deploying the app
    downsampled <- aggregate(worldpop_pop_mask, fact = 4)  # Adjust the factor according to your needs
    ...
    output$result_table <- renderTable({
      ...
      data.frame("Total_Population" = total_population_sum,
                 "Average_Population" = mean_population,
                 "Average_SPAM_Yield" = mean_yield)
    })
